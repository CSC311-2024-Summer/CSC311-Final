...
def compute_cost(alpha, beta, s, r, t, lambda_1, lambda_2):
    ...
    reg_alpha = lambda_1 * np.sum(alpha) / n
    reg_beta = lambda_2 * np.sum(beta ** 2) / n
    return loss + reg_alpha + reg_beta
...
def compute_gradients(alpha, beta, s, r, t, lambda_1, lambda_2):
    ...
    grad_alpha += lambda_1
    grad_beta += lambda_2 * np.sum(beta) / n
